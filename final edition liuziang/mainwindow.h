...
class MainWindow : public QWidget
{
    Q_OBJECT
private:
    QVector<towerposition*> TowerPitVec;  //防御塔坑数组
  ...
  //void mouseMoveEvent(QMouseEvent *);     //鼠标移动事件
    ...
    bool DeductionMoney(int);         //判断金钱是否足够并刷新标签

    int life = 10;      //生命数量

    int counter = 0;    //用于产生怪物的计数器

    const int RewardMoney = 20; //每次击败怪物获得的金钱数量

    CoorStr *homecoor = new CoorStr(0, 0);  //记录基地的坐标，从地图中自动获取
